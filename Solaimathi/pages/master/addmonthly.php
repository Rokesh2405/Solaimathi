            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="btn-group pull-right m-t-15">
                        <a href="<?php echo $sitename; ?>master/monthly.htm"><button type="button" class="btn btn-default">Back to Listings Page</button>
                        </a>
                    ...
                    <h4 class="page-title"><?php
                        ...
                        ?> Monthly Installment</h4>
                    <ol class="breadcrumb">
                      
                        <li class="breadcrumb-item"> Monthly Installment List</li>
  ...
  <br>
                                        ...
                                        <div class="box-body">
											...
											$loanamt=$getrecd['loan_amount']/$getrecd['due_amt_per_month'];
											 ...
											 $getrecd = FETCH_all("SELECT * FROM customer WHERE (`name` LIKE '%".$cusid."%') OR (`mobileno`='".$cusid."') AND `vehicle_type`='5' AND `id`!=? ORDER BY `id` DESC", '0');
											 if($getrecd['vehicle_type']=='1' || $getrecd['vehicle_type']=='2'){
											 $loanamt= $getrecd['due_amt_per_month'];
											}
											else
		  ...
		  <div class="col-md-4">
		   ...
		   <div class="row">
						  <div class="col-md-4">
							...
							<?php echo getcustomer('name',getinstallment('customerid', $_REQUEST['banid'])); ?>
							</div></div>
							<br>
		   <?php //} ?>
                          <div class="row">
						...
						<input type="date" name="paid_date" class="form-control" value="<?php if(getinstallment('paid_date', $_REQUEST['banid'])!='') { echo getinstallment('paid_date', $_REQUEST['banid']); } else { echo date('Y-m-d');} ?>">
							...
							<input type="text" required="required" name="amount" value="<?php if($loanamt!='') { echo number_format($loanamt,2); } else { echo getinstallment('amount', $_REQUEST['banid']); }?>" id="amount" placeholder="Enter the Amount" class="form-control" />
							</div>
		   ...
		   <?php //} ?>
                ...
                <div class="box-footer">
                    <div class="row">
                        <div class="col-md-6">
                            <a href="<?php echo $sitename; ?>master/weekly.htm">Back to Listings page</a>
                        </div>
...
$("#cusname").keyup(function(){
 ...
 $.ajax({
            ...
            data: {searchkey: searchkey,vehicle_type:5},
            success: function (data) {
...
$("#amount").keyup(function(){
 ...
 $.ajax({
            url: "<?php echo $sitename; ?>ajaxfunction.php",
            data: {cusid: cusid,vehicle_type:5,incval:1,amount:amount},
            success: function (data) {
			var result = data.split('#');
